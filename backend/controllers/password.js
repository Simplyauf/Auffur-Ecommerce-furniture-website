...
const resetPasswordMessageData = (passwordVerificationToken) => {
  return {
    ...
    html: `<h2>Verify your email address </h2><br/><div>We received a request to reset your password for your <b>Auffur </b> account. If you made this request, please click the link below to reset your password:</div><br/> <a href="http://localhost:5000/api/v1/auth/resetPasswordLink/password-reset?token=${passwordVerificationToken}" style="display: inline-block; padding: 0.5em 1em; background-color: #fca311; color: white; text-decoration: none;">Verify Email</a> <br /> <br /> <br /> <div>This link will expire in 1 hour.If you did not make this request, you can safely ignore this email,your password will not be reset and your account will remain secure.
</div> <br /> <span>Thanks,</span> <br /> <span>Auffur Team</span>`,
  };
};

// This fires when "forgot password" link is clicked in frontend,it assumes the user email should be in login field
const handleForgotPasswordClick = async (req, res) => {
  const { email } = req.body;
  let checkIfEmailExists = await User.findOne({ email });

  if (!checkIfEmailExists) {
    throw new CustomErrorHandler(400, "Email address doesnt exist");
  } else if (checkIfEmailExists.verificationStatus !== "verified") {
    throw new CustomErrorHandler(401, "User account must be verified");
  }
...
// This will be used for updating the  password in the frontend
const changePassword = async (req, res) => {
  let token = req.query.token;
  const { password } = req.body;
  let checkIfTokenExist = User.findOne({ verificationToken: token });
